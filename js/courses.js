var courses = {
  ...
  "Elective - will be core in future": {
...
function getSkillmap() {
    for (track in courses){
        for (course in courses[track]){
            ...
            for(skill in courses[track][course]["Pre-req"]){
                var level = courses[track][course]["Pre-req"][skill];
                var ID = courses[track][course]["courseID"];
                var _course = {courseName:course, courseID:ID, track: track};
                // console.log(_course)

                if (skillMap_pre[skill] == undefined) {
                    skillMap_pre[skill] = {};
                    skillMap_pre[skill][level] = [];
                    skillMap_pre[skill][level].push(_course);
                    // console.log("???", skillMap_pre[skill]);
                    // console.log("kkk", skillMap_pre);
                } else {
                    if (skillMap_pre[skill][level] == undefined) {
                        skillMap_pre[skill][level] = [];
                        skillMap_pre[skill][level].push(_course);
                    } else {
                        skillMap_pre[skill][level].push(_course);
...
getSkillmap();
// console.log("....", skillMap_out);
// console.log("====", skillMap_pre);

function getCourseName(skill, courseID) {
    var tem = skillMap_out[skill];
    for (level in tem) {
        for (i = 0; i < tem[level].length; i++) {
          if (tem[level][i].courseID == courseID) {return tem[level][i].courseName;}  
        }
    }
    return "Not Find";
}
...
function getCourseNameI(skill, courseID) {
  var tem = skillMap_pre[skill];
  for (level in tem) {
      for (i = 0; i < tem[level].length; i++) {
        if (tem[level][i].courseID == courseID) {return tem[level][i].courseName;}  
      }
